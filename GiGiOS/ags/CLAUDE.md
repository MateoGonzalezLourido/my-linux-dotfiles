...
- `modulos/ajustes-rapidos/QuickSettings.tsx` — large control panel (Wi-Fi, Bluetooth, audio, display). Al importarse instala el **switch-on-connect de audio**: al conectar un dispositivo, pasa a ser la salida/entrada por defecto. **`speaker-added` NO significa "alguien ha enchufado algo"**, y creerlo costó dos bugs distintos; los dos dejaban el sonido en el **HDMI de la GPU**, que en esta máquina no tiene nada conectado (es una salida interna), y como el default así fijado **se persiste** (`default.configured.audio.sink` en `~/.local/state/wireplumber/default-nodes`), el estropicio sobrevivía al reinicio y pisaba lo que WirePlumber ya había restaurado bien:
  ...
  **El volumen por aplicación no se aplicaba a las apps lanzadas con el panel cerrado** (`servicios/multimedia/presetsApps.ts`). Los presets se guardaban bien en `audioPresets.json`, pero la única línea que los **aplicaba** vivía dentro del sondeo `pactl` de la "mezcla de aplicaciones", y ese sondeo tiene refcount contra `quickSettingsVisible ∧ qsView ∧ audioMode === "apps"`: solo corre con ese submenú **abierto**. O sea que bajabas Spotify al 57 %, cerrabas el panel, reabrías Spotify y sonaba al volumen que trajera la app; el ajuste no se había perdido —seguía en el JSON, y al abrir el submenú se aplicaba de golpe— simplemente no había nadie escuchando cuando aparecía el stream. Silencioso de manual: ningún error, y con el panel abierto (que es como se depura) funciona siempre. No era específico de Spotify. Hoy el almacén de presets y su aplicación viven en `presetsApps.ts`, con `initPresetsApps()` en el bloque de t=4 s de `app.ts`, y `QuickSettings.tsx` se queda con la UI:
  - **Por eventos, no por sondeo**: un único `pactl subscribe` que duerme hasta que PipeWire habla, en vez de dejar dos `pactl` cada 2 s toda la sesión por si acaso. Se lanza con **`LC_ALL=C`** porque sus mensajes son traducibles y el parseo de `Event 'new' on sink-input #45` dependería del locale. Se reengancha con espera creciente (techo 30 s) si `pipewire-pulse` se reinicia y se lleva el subscribe por delante.
  - **Segunda pasada a 700 ms.** Una app puede fijar su propio volumen **justo después** de crear el stream (Spotify restaura el suyo al conectar). Aplicando solo en el evento `new`, el cliente gana la carrera y el preset vuelve a no verse — el mismo síntoma con otra causa. Medido: pisar el stream a 100 % a los 450 ms y verlo volver al 20 % del preset.
  - **El set de índices atendidos se PODA** contra los streams vivos en cada pasada. PulseAudio recicla los índices de sink-input, y un índice reciclado que siguiera marcado como atendido no volvería a recibir su preset nunca.
  - **El hijo NO se muere solo con AGS, y por eso `pactl subscribe` va envuelto en un `bash`.** Gio no mata a los hijos al salir el proceso, y el mecanismo que sí los mata de rebote —SIGPIPE al escribir en un pipe cuyo lector ya no está— aquí no dispara: este proceso solo escribe **cuando ocurre un evento de audio**, así que un AGS reiniciado en silencio deja un huérfano por reinicio (medido: cuatro vivos con un solo AGS). `cava` no tiene el problema porque escribe 60 líneas por segundo y se lleva su SIGPIPE en el primer frame; de ahí que esto no se hubiera visto antes. El envoltorio vigila el STDIN que nos une a AGS: al morir AGS ese pipe se cierra, `cat` ve EOF y mata a `pactl`; y el `wait -n` sobre los dos hace que, si el que cae es `pactl` (reinicio de `pipewire-pulse`), el envoltorio termine también y cierre su stdout — que es justo el EOF con el que nuestro lector reengancha. **`exec 3<&0` no es adorno**: POSIX manda que un shell sin control de trabajos redirija a `/dev/null` el stdin de todo proceso en segundo plano, así que un `cat > /dev/null &` a secas ve EOF en el acto y mata a `pactl` nada más arrancar — sin un solo error por ninguna parte, con el vigilante muerto y el fallo original intacto.
  - **La clave del preset (`clavePreset`/`nombreStream`) la pone el servicio, no cada consumidor** (el nombre VISIBLE es otra cosa: ver el bloque siguiente). Estaba duplicada a mano en la fila de la UI y en el poller, y esa duplicación ya costó un fallo: la fila de micro guardaba con `mic:` y el poller aplicaba con `app:mic:`, así que el preset no se releía. El sondeo de la UI sigue existiendo (alimenta la lista) y llama a `aplicarPresets`, que es idempotente: con el panel abierto atiende antes que el vigilante, sin doble `set-sink-input-volume`.
